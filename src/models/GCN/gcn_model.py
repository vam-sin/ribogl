...
parser.add_argument('--random_walk', type=bool, default=False, help='addition of random walk embedding')
parser.add_argument('--scheduler_alg', type=str, default='R', help='learning rate scheduler to use') # CosineAnneal, Regular
parser.add_argument('--model_algo', type=str, default='GCNOnly', help='model algorithm') # GCN_LSTM, GCNOnly, Ensemble, LSTMEmbedswGCN
...
parser.add_argument('--algo', type=str, default='Cheb', help='convolution algorithm') # SAGE, GAT, GATv2, GINE, TF, DeepGCN (which uses TF)
...
model_name = scheduler_alg + ' ' + str(model_algo) + ' ' + model_type + '-' + algo + '[BS ' + str(batch_size) + ', D ' + str(dropout_val) + ', E ' + str(tot_epochs) + ', LR ' + str(lr) + ', VN ' + str(virtual_node) + ', RW ' + str(random_walk) + ', APE: ' + str(abs_pos_enc) +  ', CK: ' + str(cheb_k) + ']' + 'L: ' + str(gcn_layers) + ' Capr: ' + str(capr) + ' CaLM: ' + str(calm_bool)
